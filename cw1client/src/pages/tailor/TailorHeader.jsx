...
import { getCurrentStaff } from "../../api/tailor";
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="px-2">
            ...
            <p className="text-gray-500 text-sm sm:text-base">
              Welcome back,&nbsp;
              <span className="font-semibold text-pink-500">{tailor.name}</span>
            </p>
          </div>
        </div>
              ...
              {tailor.profileImage ? (
                ...
                <div className="w-11 h-11 bg-pink-500 rounded-full flex items-center justify-center border-2 border-gray-200">
                  <span className="text-white font-semibold text-lg">
                    {tailor.name?.charAt(0).toUpperCase() || "C"}
                  </span>
                </div>
              )}
      ...
      {showProfile && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg relative w-[95%] sm:max-w-md max-h-[90vh] overflow-y-auto">
            {/* <CutterProfile tailor={tailor} setShowProfile={setShowProfile} /> */}
          </div>
        </div>
      )}
    </div>
